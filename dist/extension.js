...
var CodeEditor = class _CodeEditor {
  static diffProvider = null;
  /**
   * Register the diff content provider (call this in extension activation)
   */
  static registerDiffProvider(context) {
    if (!_CodeEditor.diffProvider) {
      _CodeEditor.diffProvider = new DiffContentProvider();
      context.subscriptions.push(
        vscode.workspace.registerTextDocumentContentProvider("devalley-diff", _CodeEditor.diffProvider)
      );
  ...
  static findTextRangeFlexible(document2, needleRaw) {
    if (!needleRaw) return null;
    const fullRaw = document2.getText();
    const full = _CodeEditor.normalizeText(fullRaw);
    const needle = _CodeEditor.normalizeText(needleRaw);
    let idx = full.indexOf(needle);
    if (idx >= 0) {
      const start = document2.positionAt(_CodeEditor.mapNormalizedIndexToRaw(fullRaw, idx));
      const end = document2.positionAt(
        _CodeEditor.mapNormalizedIndexToRaw(fullRaw, idx + needle.length)
      );
    ...
    if (idx >= 0) {
      const start = document2.positionAt(_CodeEditor.mapNormalizedIndexToRaw(fullRaw, idx));
      const end = document2.positionAt(
        _CodeEditor.mapNormalizedIndexToRaw(fullRaw, idx + needleNoLastNewline.length)
      );
      return new vscode.Range(start, end);
    }
    return null;
   ...
   */
  static mapNormalizedIndexToRaw(raw, normalizedIndex) {
    let n = 0;
   ...
   */
  static resolveToAbsoluteFsPath(inputPath) {
  ...
  static toFileUriFromEdit(edit) {
    const abs = _CodeEditor.resolveToAbsoluteFsPath(edit.filePath);
    return vscode.Uri.file(abs);
  }
  static async fileExists(fileUri) {
    try {
  ...
  static async ensureDirExists(dirUri) {
    try {
      await vscode.workspace.fs.createDirectory(dirUri);
    } catch {
    }
  }
  static normalizeEdits(edits) {
    return (edits || []).map((e) => ({
      op: e.op ?? "modify",
      ...e
    }));
  }
  static makeVirtualUri(kind, absPath) {
   ...
   */
  static buildModifiedContent(originalContent, startLine, endLine, newText) {
   ...
   */
  static extractOldBlock(originalContent, startLine, endLine) {
    const lines = originalContent.split("\n");
    const safeStart = Math.max(0, Math.min(startLine, lines.length - 1));
    const safeEnd = Math.max(safeStart, Math.min(endLine, lines.length - 1));
   ...
   */
  static async showDiffPreview(edits) {
    ...
    const normalized = _CodeEditor.normalizeEdits(edits);
    for (const edit of normalized) {
      try {
        const op = edit.op ?? "modify";
        const fileUri = _CodeEditor.toFileUriFromEdit(edit);
        const absPath = fileUri.fsPath;
        const exists = await _CodeEditor.fileExists(fileUri);
        console.log("[DevAlley] Preview:", { op, input: edit.filePath, resolved: absPath, exists });
        if (op === "create") {
          const content = edit.newText ?? "";
          const left = _CodeEditor.makeVirtualUri("create-empty", absPath);
          const right = _CodeEditor.makeVirtualUri("create-new", absPath);
          _CodeEditor.diffProvider.update(left, "");
          _CodeEditor.diffProvider.update(right, content);
        ...
        if (op === "delete") {
          ...
          const originalDocument2 = await vscode.workspace.openTextDocument(fileUri);
          const originalContent2 = originalDocument2.getText();
          const empty = _CodeEditor.makeVirtualUri("delete-empty", absPath);
          _CodeEditor.diffProvider.update(empty, "");
          await vscode.commands.executeCommand(
            "vscode.diff",
            fileUri,
            empty,
        ...
        if (typeof edit.oldText === "string" && edit.oldText.length > 0) {
          const actual = _CodeEditor.extractOldBlock(originalContent, startLine, endLine);
        ...
        const { modifiedContent } = _CodeEditor.buildModifiedContent(
          originalContent,
          startLine,
          endLine,
          edit.newText ?? ""
        );
        const modifiedUri = _CodeEditor.makeVirtualUri("modify", absPath);
        _CodeEditor.diffProvider.update(modifiedUri, modifiedContent);
        await vscode.commands.executeCommand(
          "vscode.diff",
          fileUri,
          modifiedUri,
   ...
   */
  static deduplicateEdits(document2, edits) {
    if (edits.length <= 1) return edits;
    const rangeMap = /* @__PURE__ */ new Map();
    for (const edit of edits) {
      let rangeKey;
      if (typeof edit.oldText === "string" && edit.oldText.trim().length > 0) {
        const found = _CodeEditor.findTextRangeFlexible(document2, edit.oldText);
        if (found) {
  ...
  static async applyEdits(edits) {
    ...
    const modifiesByFile = /* @__PURE__ */ new Map();
    for (const edit of modifies) {
      const absPath = _CodeEditor.resolveToAbsoluteFsPath(edit.filePath);
    ...
    try {
      for (const edit of creates) {
        ...
        await _CodeEditor.ensureDirExists(dirUri);
      ...
      for (const [absPath, fileEdits] of modifiesByFile) {
        ...
        const deduplicatedEdits = _CodeEditor.deduplicateEdits(document2, fileEdits);
        for (const edit of deduplicatedEdits) {
          const startLine = Number.isFinite(Number(edit.startLine)) ? Number(edit.startLine) : 0;
          const endLine = Number.isFinite(Number(edit.endLine)) ? Number(edit.endLine) : startLine;
          if (startLine >= document2.lineCount) {
            throw new Error(`Invalid startLine ${startLine}. File has ${document2.lineCount} lines.`);
          }
          const safeEndLine = Math.min(Math.max(endLine, startLine), document2.lineCount - 1);
          let range = null;
          if (typeof edit.oldText === "string" && edit.oldText.trim().length > 0) {
            const found = _CodeEditor.findTextRangeFlexible(document2, edit.oldText);
            if (found) {
              ...
              let end;
              if (found.end.line + 1 < document2.lineCount) {
                end = new vscode.Position(found.end.line + 1, 0);
              } else {
                end = new vscode.Position(found.end.line, document2.lineAt(found.end.line).text.length);
              }
          ...
          if (!range) {
            ...
            let endPos;
            if (safeEndLine + 1 < document2.lineCount) {
              endPos = new vscode.Position(safeEndLine + 1, 0);
            } else {
              endPos = new vscode.Position(safeEndLine, document2.lineAt(safeEndLine).text.length);
            }
...
function getHtmlForWebview(webview, extensionUri) {
  ...
  const logo = webview.asWebviewUri(
    vscode2.Uri.joinPath(extensionUri, "media", "Adroitent_logo.png")
  );
  const scriptUri = webview.asWebviewUri(
    vscode2.Uri.joinPath(extensionUri, "media", "main.js")
  );
  const styleUri = webview.asWebviewUri(
    vscode2.Uri.joinPath(extensionUri, "media", "styles.css")
  );
...
function buildHtmlContent(nonce, logoUrl, scriptUri, styleUri) {
...
function getBodyContent(logoUrl) {
  return `<div class="app-container">
    ...
    <button id="activityClose" class="activity-close" title="Dismiss">\xD7</button>
  </div>
      <!-- Login -->
          ...
          <div id="fileContextContainer" style="display: none;">
            <div class="file-context-display">
              <div class="file-context-header">
                ...
                <button class="file-context-close" id="clearContextBtn" title="Remove file context">\xD7</button>
              </div>
    ...
    <div id="settingsPanel" class="settings-panel hidden">
      <div class="settings-header">
        ...
        <button id="closeSettingsBtn" class="close-settings-btn">\xD7</button>
      </div>
      
      <div class="settings-content">
        <div class="settings-section">
          <h3 class="settings-section-title">
            <span class="section-icon">\u{1F4DD}</span>
            Context Mode
          </h3>
...
var WorkspaceIndexer = class {
  ...
  apiBaseUrl = "http://202.153.39.93:7067";
  // Extensions to IGNORE (binary/generated files)
  ignoreExtensions = /* @__PURE__ */ new Set([
    ".exe",
    ".dll",
    ".so",
    ".dylib",
  ...
  async generateWorkspaceId() {
    if (!this.workspacePath) return "unknown";
    const folderName = path2.basename(this.workspacePath);
    const workspaceKey = `workspace_id_${this.workspacePath}`;
    const allKeys = this.context.workspaceState.keys();
    console.log("[Indexer] \u{1F511} All workspaceState keys:", allKeys);
    console.log("[Indexer] Looking for key:", workspaceKey);
    let workspaceId = this.context.workspaceState.get(workspaceKey);
    if (!workspaceId) {
      const pathHash = require("crypto").createHash("md5").update(path2.resolve(this.workspacePath)).digest("hex").slice(0, 8);
      workspaceId = `${folderName}_${pathHash}`;
      await this.context.workspaceState.update(workspaceKey, workspaceId);
      console.log(`[Indexer] \u{1F195} Generated NEW workspace ID: ${workspaceId}`);
  ...
  getAuthHeaders() {
    const token = this.context.globalState.get("devAlleyToken");
  ...
  async ensureConnected() {
    ...
    if (!token) {
      ...
      vscode3.window.showErrorMessage("Please login to Devailey first");
      return false;
    }
    try {
      ...
      if (error.response?.status === 401) {
        vscode3.window.showErrorMessage("Devailey: Authentication failed. Please login again.");
      } else if (error.response?.status) {
        vscode3.window.showErrorMessage(`Devailey: Backend error (${error.response.status}). Check server logs.`);
      } else {
        vscode3.window.showErrorMessage(`Devailey: Cannot connect to backend. Is the server running?`);
      }
      return false;
    }
  }
  async indexWorkspace(progressCallback) {
    ...
    progressCallback?.("\u{1F517} Connecting to backend...");
    if (!await this.ensureConnected()) {
      progressCallback?.("\u274C Connection failed");
      return 0;
    }
    const startTime = Date.now();
    progressCallback?.("\u{1F4C1} Scanning workspace...");
    ...
    for (const filePath of files) {
      try {
        ...
        const relativePath = path2.relative(this.workspacePath, filePath);
        console.log(`[Indexer] Indexing ${path2.basename(filePath)} (${content.length} chars)`);
        console.log(`[Indexer] - Absolute path: ${filePath}`);
        console.log(`[Indexer] - Relative path: ${relativePath}`);
        const url2 = `${this.apiBaseUrl}/api/vscode/index/file`;
        ...
        console.log("[Indexer] POST URL:", url2);
        ...
        if (response.data.success) {
          ...
          console.log(`[Indexer] \u2705 ${path2.basename(filePath)} -> ${chunksAdded} chunks`);
        } else {
        ...
        failedFiles++;
        console.error(`[Indexer] \u274C Error indexing ${path2.basename(filePath)}:`, error.message);
        if (error.response) {
  ...
  async findRelevantContext(query, topK = 15) {
    if (!this.workspacePath) {
...
`;
      for (const result of results) {
        ...
        const relativePath = path2.relative(this.workspacePath, filePath);
        const fileMarker = !seenFiles.has(relativePath) ? "\u{1F4C4} NEW FILE\n" : "";
...
`);
  ...
  async shouldIndexFile(filePath, fileName) {
...
// src/devAlleyPanel.ts
var DevAlleyPanel = class _DevAlleyPanel {
  static currentPanel;
  panel;
  context;
  disposables = [];
  ...
  static indexer = null;
  static TOKEN_KEY = "devalley.token";
  static USERNAME_KEY = "devalley.username";
  static AUTO_CAPTURE_KEY = "devalley.autoCapture";
  static PREF_MODE_KEY = "devalley.pref.contextMode";
  static EDIT_MODE_KEY = "devalley.editMode";
  static async createOrShow(context, forceLogin = false) {
    const column = vscode4.ViewColumn.Beside;
    if (_DevAlleyPanel.currentPanel) {
      _DevAlleyPanel.currentPanel.panel.reveal(column);
      console.log("[DevAlley] Panel already exists, just revealing");
      return;
    }
    if (!_DevAlleyPanel.indexer) {
      console.log("[DevAlley] Initializing workspace indexer...");
      _DevAlleyPanel.indexer = new WorkspaceIndexer(context);
      await _DevAlleyPanel.indexer.initialize();
    ...
    _DevAlleyPanel.currentPanel = new _DevAlleyPanel(panel, context, forceLogin);
  }
  static toggle(context) {
    if (_DevAlleyPanel.currentPanel) {
      _DevAlleyPanel.currentPanel.dispose();
    } else {
      _DevAlleyPanel.createOrShow(context, false);
    }
  }
  static revive(panel, context, state) {
    _DevAlleyPanel.currentPanel = new _DevAlleyPanel(panel, context, false);
  }
  constructor(panel, context, forceLogin = false) {
    this.panel = panel;
    this.context = context;
      ...
      async (message) => {
        try {
          ...
          switch (message?.type) {
            ...
            // ===== FILE CONTEXT =====
            case "getFileContext": {
              const requestedMode = message?.mode || "snippet";
            ...
            case "indexWorkspace": {
              ...
              await _DevAlleyPanel.indexer.indexWorkspace((msg) => {
                this.safePost({ type: "indexing:progress", message: msg });
              });
              const stats = await _DevAlleyPanel.indexer.getStats();
              this.safePost({
                type: "indexing:complete",
                stats
              });
            ...
            case "getWorkspaceContext": {
              ...
              const contextText = await _DevAlleyPanel.indexer.findRelevantContext(
                message.query || "",
                15
              );
              console.log(
            ...
            case "previewCodeEdits": {
              ...
              this.sendAssistantLog(
                `\u{1F441}\uFE0F Previewing ${edits.length} change(s)\u2026`
              );
              await CodeEditor.showDiffPreview(edits);
              this.pendingEdits = edits;
              this.sendAssistantLog("\u{1F441}\uFE0F Preview opened.");
              break;
            }
            case "applyCodeEdits": {
              const edits = message.edits || [];
            ...
            case "auth:set": {
              ...
              await this.context.secrets.store(_DevAlleyPanel.TOKEN_KEY, token);
              await this.context.secrets.store(
                _DevAlleyPanel.USERNAME_KEY,
                username
              );
            ...
            case "webview:ready": {
              ...
              const savedMode = this.context.globalState.get(
                _DevAlleyPanel.PREF_MODE_KEY
              ) || "snippet";
              ...
              const savedEditMode = this.context.globalState.get(
                _DevAlleyPanel.EDIT_MODE_KEY
              ) || false;
              this.safePost({
                type: "editMode:state",
  ...
  async loadAutoCapturePreference() {
    const stored = this.context.globalState.get(
      _DevAlleyPanel.AUTO_CAPTURE_KEY
    );
  ...
  async saveAutoCapturePreference() {
    await this.context.globalState.update(
      _DevAlleyPanel.AUTO_CAPTURE_KEY,
  ...
  startAutoCapture() {
    ...
    this.editorChangeListener = vscode4.window.onDidChangeActiveTextEditor(
      (editor) => {
  ...
  stopAutoCapture() {
    if (this.editorChangeListener) {
      ...
      this.editorChangeListener = void 0;
    }
  }
  // ===== File context =====
  getFileContext(mode = "snippet") {
    const editor = vscode4.window.activeTextEditor;
    if (!editor) return null;
    const document2 = editor.document;
    const selection = editor.selection;
    const workspaceFolder = vscode4.workspace.getWorkspaceFolder(document2.uri);
    const relPath = workspaceFolder ? vscode4.workspace.asRelativePath(document2.uri, false) : document2.fileName;
    const selectedText = document2.getText(selection) || "";
    const cursorLine = selection.active.line;
    const snippetStartLine = Math.max(0, cursorLine - 50);
    const snippetEndLine = Math.min(document2.lineCount - 1, cursorLine + 50);
    const snippetRange = new vscode4.Range(
      snippetStartLine,
      0,
      snippetEndLine,
      document2.lineAt(snippetEndLine).text.length
    );
    const snippetText = document2.getText(snippetRange) || "";
  ...
  // ===== Auth =====
  async forceLogout() {
    await this.context.secrets.delete(_DevAlleyPanel.TOKEN_KEY);
    await this.context.secrets.delete(_DevAlleyPanel.USERNAME_KEY);
    await this.context.globalState.update("devAlleyToken", null);
    console.log("[DevAlley] Auth credentials cleared");
  }
  async postAuthState() {
    try {
      const token = await this.context.secrets.get(_DevAlleyPanel.TOKEN_KEY);
      const username = await this.context.secrets.get(_DevAlleyPanel.USERNAME_KEY);
  ...
  // ===== Editor insert =====
  async insertCodeIntoEditor(content) {
    if (!content || typeof content !== "string" || !content.trim()) {
      vscode4.window.showWarningMessage("No code content to insert.");
      this.safePost({ type: "export:error", error: "No code content to insert" });
      return;
    }
    const editor = vscode4.window.activeTextEditor;
    if (!editor) {
      vscode4.window.showWarningMessage("No active editor found. Please open a file first.");
      this.safePost({ type: "export:error", error: "No active editor" });
      return;
    }
    try {
      await editor.edit((edit) => edit.insert(editor.selection.active, content));
      vscode4.window.showInformationMessage("Code inserted successfully!");
      ...
      vscode4.window.showErrorMessage("Failed to insert code: " + errMsg);
  ...
  // ===== Dispose =====
  dispose() {
    _DevAlleyPanel.currentPanel = void 0;
    this.stopAutoCapture();
    try {
      this.panel.dispose();
    } catch {
...
// src/devAlleyViewProvider.ts
var vscode5 = __toESM(require("vscode"));
var DevAlleyViewProvider = class _DevAlleyViewProvider {
  constructor(context) {
    this.context = context;
  }
  static viewType = "devalley.chatView";
  _view;
  disposables = [];
  autoCaptureEnabled = false;
  editorChangeListener;
  ...
  static TOKEN_KEY = "devalley.token";
  static USERNAME_KEY = "devalley.username";
  static REFRESH_TOKEN_KEY = "devalley.refreshToken";
  static AUTO_CAPTURE_KEY = "devalley.autoCapture";
  static PREF_MODE_KEY = "devalley.pref.contextMode";
  static EDIT_MODE_KEY = "devalley.editMode";
  async resolveWebviewView(webviewView, context, _token) {
    this._view = webviewView;
    if (!_DevAlleyViewProvider.indexer) {
      console.log("[DevAlley] Initializing workspace indexer...");
      _DevAlleyViewProvider.indexer = new WorkspaceIndexer(this.context);
      await _DevAlleyViewProvider.indexer.initialize();
      console.log("[DevAlley] Workspace indexer ready");
    }
        ...
        try {
          ...
          switch (message?.type) {
            case "prefs:set": {
              ...
              if (key === "contextMode") {
                await this.context.globalState.update(
                  _DevAlleyViewProvider.PREF_MODE_KEY,
                  value
                );
              ...
              if (key === "editMode") {
                await this.context.globalState.update(
                  _DevAlleyViewProvider.EDIT_MODE_KEY,
                  value
                );
            ...
            case "indexWorkspace": {
              ...
              this.safePost({ type: "indexing:started" });
              await _DevAlleyViewProvider.indexer.indexWorkspace((msg) => {
                this.safePost({ type: "indexing:progress", message: msg });
              });
              const stats = await _DevAlleyViewProvider.indexer.getStats();
              this.safePost({
                type: "indexing:complete",
                stats
              });
            ...
            case "getWorkspaceContext": {
              ...
              const contextText = await _DevAlleyViewProvider.indexer.findRelevantContext(
                message.query || "",
                15
              );
              this.safePost({
            ...
            case "previewCodeEdits": {
              const edits = message.edits || [];
              console.log("[DevAlley] Previewing", edits.length, "code edits");
              this.sendAssistantLog(`\u{1F441}\uFE0F Previewing ${edits.length} change(s)\u2026`);
              await CodeEditor.showDiffPreview(edits);
              this.pendingEdits = edits;
              this.sendAssistantLog("\u{1F441}\uFE0F Preview opened.");
              break;
            ...
            case "auth:set": {
              ...
              await this.context.secrets.store(
                _DevAlleyViewProvider.TOKEN_KEY,
                token
              );
              await this.context.secrets.store(
                _DevAlleyViewProvider.USERNAME_KEY,
                username
              );
              if (refreshToken) {
                await this.context.secrets.store(
                  _DevAlleyViewProvider.REFRESH_TOKEN_KEY,
                  refreshToken
                );
              }
              await this.context.globalState.update("devAlleyToken", token);
  ...
  async syncStateToWebview() {
    ...
    if (savedMode === "file" || savedMode === "snippet") {
      const fileCtx = this.getFileContext(savedMode);
      if (fileCtx) {
        this.safePost({
          type: "fileContext:response",
          fileContext: fileCtx,
  ...
  async loadAutoCapturePreference() {
    const stored = this.context.globalState.get(
      _DevAlleyViewProvider.AUTO_CAPTURE_KEY
    );
    this.autoCaptureEnabled = stored ?? false;
    if (this.autoCaptureEnabled) {
      this.startAutoCapture();
  ...
  startAutoCapture() {
    if (this.editorChangeListener) return;
    this.editorChangeListener = vscode5.window.onDidChangeActiveTextEditor(
      (editor) => {
  ...
  getFileContext(mode = "snippet") {
    ...
    const document2 = editor.document;
    const selection = editor.selection;
    const workspaceFolder = vscode5.workspace.getWorkspaceFolder(document2.uri);
    const relPath = workspaceFolder ? vscode5.workspace.asRelativePath(document2.uri, false) : document2.fileName;
    const selectedText = document2.getText(selection) || "";
    const cursorLine = selection.active.line;
    const snippetStartLine = Math.max(0, cursorLine - 50);
    const snippetEndLine = Math.min(document2.lineCount - 1, cursorLine + 50);
    const snippetRange = new vscode5.Range(
      snippetStartLine,
      0,
      snippetEndLine,
      document2.lineAt(snippetEndLine).text.length
  ...
  async forceLogout() {
    await this.context.secrets.delete(_DevAlleyViewProvider.TOKEN_KEY);
    await this.context.secrets.delete(_DevAlleyViewProvider.USERNAME_KEY);
    await this.context.secrets.delete(_DevAlleyViewProvider.REFRESH_TOKEN_KEY);
    await this.context.globalState.update("devAlleyToken", null);
  }
  async postAuthState() {
    try {
  ...
  async insertCodeIntoEditor(content) {
    if (!content || typeof content !== "string" || !content.trim()) {
      vscode5.window.showWarningMessage("No code content to insert.");
      this.safePost({ type: "export:error", error: "No code content" });
      return;
    }
    const editor = vscode5.window.activeTextEditor;
    if (!editor) {
      vscode5.window.showWarningMessage("No active editor found.");
      this.safePost({ type: "export:error", error: "No active editor" });
      return;
    }
    try {
      await editor.edit(
        (edit) => edit.insert(editor.selection.active, content)
      );
      vscode5.window.showInformationMessage("Code inserted successfully!");
      this.safePost({ type: "export:success" });
    } catch (err) {
      vscode5.window.showErrorMessage("Failed to insert code");
...
function activate(context) {
  ...
  if (vscode6.window.registerWebviewPanelSerializer) {
    vscode6.window.registerWebviewPanelSerializer("devalleyAssistant", {
  ...
  context.subscriptions.push(
    vscode6.commands.registerCommand("devalley.openAssistant", () => {
      DevAlleyPanel.createOrShow(context, false);
    })
  );
  context.subscriptions.push(
    vscode6.commands.registerCommand("devalley.toggleAssistant", () => {
      DevAlleyPanel.toggle(context);
    })
  );
  context.subscriptions.push(
    vscode6.commands.registerCommand("devalley.focusAssistantView", async () => {
      await vscode6.commands.executeCommand("workbench.view.extension.devalley");
      await vscode6.commands.executeCommand("devalley.chatView.focus");
      await vscode6.commands.executeCommand("workbench.action.focusSideBar");
    })
  );
}
function deactivate() {
